  ...
  return (
    <div className="p-4 md:p-6 space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black neon-glow uppercase italic tracking-tighter">Contactos</h1>
          <p className="text-slate-500 text-sm font-medium">Gestión centralizada de clientes y proveedores.</p>
        </div>
        <button 
        ...
        <div className="glass p-8 rounded-[2rem] border border-primary-500/20 animate-in zoom-in-95">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm font-bold text-white outline-none focus:border-primary-500 shadow-inner"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1">Descripción / Notas</label>
                <input 
                  type="text"
            ...
            <div className="flex gap-4">
              <button type="submit" className="flex-1 py-4 bg-primary-600 text-white font-black uppercase text-xs rounded-xl shadow-lg">Guardar Contacto</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPartners.map(p => (
          <div key={p.id} className="glass p-6 rounded-[2rem] border border-white/5 hover:border-primary-500/30 transition-all group shadow-xl">
            ...
            <div className="flex items-center gap-4 pt-4 border-t border-white/5">
              ...
              <div className="flex items-center gap-1.5 text-[10px] font-black text-emerald-500 uppercase ml-auto">
        ...
        {filteredPartners.length === 0 && (
          <div className="col-span-full py-20 text-center">
             ...
             <p className="text-slate-600 font-black uppercase tracking-[0.4em] text-xs">No hay contactos registrados</p>
          </div>
        )}
      </div>
    </div>
