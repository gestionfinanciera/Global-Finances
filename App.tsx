    ...
    <div className="min-h-screen flex flex-col md:flex-row relative">
      <header className="md:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-primary-500/20 sticky top-0 z-30 backdrop-blur-md">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-primary-600 rounded-lg flex items-center justify-center text-white font-black shadow-lg">G</div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-900 border-r border-primary-500/20 transform transition-transform duration-300 md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 px-6 py-4 space-y-1 overflow-y-auto scrollbar-thin">
            {navItems.map(item => (
              <button key={item.id} onClick={() => { setActiveTab(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 ${activeTab === item.id ? 'bg-primary-600/10 text-primary-500 font-black shadow-[inset_0_0_10px_rgba(255,0,85,0.1)] border border-primary-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}>
          ...
          <div className="p-6 border-t border-white/5">
            <button onClick={() => setIsEntryModalOpen(true)} className="w-full flex items-center justify-center gap-3 bg-primary-600 hover:bg-primary-500 text-white font-black py-4 px-6 rounded-2xl shadow-2xl transition-all transform active:scale-95 uppercase text-xs tracking-[0.2em]">
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-4 z-50">
        <button onClick={() => setIsScannerOpen(true)} className="w-14 h-14 bg-slate-800 border border-primary-500/30 rounded-full flex items-center justify-center shadow-2xl text-primary-400 hover:bg-slate-700 transition-all active:scale-90" title="Escanear">
          <ScanLine size={28} />
        </button>
        <button onClick={() => setIsChatOpen(true)} className="w-16 h-16 bg-primary-600 text-white rounded-full flex items-center justify-center shadow-[0_10px_30px_rgba(255,0,85,0.4)] hover:bg-primary-500 transition-all transform hover:scale-105 active:scale-90 border-2 border-primary-400/20" title="Asistente">
